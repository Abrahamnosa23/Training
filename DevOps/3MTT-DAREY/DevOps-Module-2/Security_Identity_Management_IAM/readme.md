...
Use the Visual editor for ease:
...
Description: Allows full access to EC2 instances for backend development teams.
...
Description: Allows full access to S3 buckets and objects for the data analysis team.
...
Under Attach permissions policies, search for and select the custom developer-policy you created.
...
Under Attach permissions policies, search for and select the custom analyst-policy you created.
...
Select AWS credential type: Ensure Provide user access to the AWS Management Console is selected. This generates a password and a custom sign-in link.

Set a Custom password and check User must create a new password at next sign-in. This is a security best practice.

Click Next.

On the Add user to group page, select the Development-Team group. Do not add the user directly to a policy; always use groups for permission management.

Click Next, review the details, and click Create user.

CRITICAL: Download the .csv file containing John's sign-in URL, username, and temporary password. This is the only time you can download these credentials.
...
On the Add user to group page, select the Analyst-Team group.

Create the user and download her credentials.
...
While signed in as an administrator, go to IAM Console > Users.

Click on the user john.

Go to the Security credentials tab.

In the Multi-factor authentication (MFA) section, click Enable.
...
In the Authentication code 1 field, enter the 6-digit code currently displayed in your authenticator app.

Wait for the code to refresh (about 30 seconds), then enter the next code in the Authentication code 2 field.
...
Log in to the AWS Console using John's unique sign-in URL and temporary password. You will be forced to set a new password.

Navigate to the EC2 Dashboard. Verify that John can successfully view, launch, stop, and terminate EC2 instances. This confirms the developer-policy is working.

Attempt to access the S3 Dashboard. This attempt should fail with an explicit "Access Denied" or "You are not authorized" message. This is the desired behavior, confirming that John cannot access services outside his job requirements.

Testing Mary's Access (Data Analyst)
Log in to the AWS Console using Mary's unique sign-in URL and temporary password. Set a new password.

Navigate to the S3 Dashboard. Verify that Mary can view buckets, create new buckets, and upload files.

Attempt to access the EC2 Dashboard. This attempt should also fail, confirming her permissions are correctly restricted to only S3.
