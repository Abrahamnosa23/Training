...
IAM Permissions: The AWS user/role configured for the CLI must have a policy granting permissions for:
...
Key Pair: An existing EC2 Key Pair in your target AWS region.

Security Group: An existing Security Group configured to allow intended traffic (e.g., SSH on port 22).
...
Enter a name (e.g., my-automation-key).

Choose key pair type: RSA and Private key file format: .pem.

Click Create key pair. The .pem file will download automatically. Store it in a secure location (e.g., ~/.ssh/).
...
Follow Code Style: Adhere to existing indentation (2 spaces), commenting style, and structure.

Test Your Changes: Validate your script changes in a non-production AWS account.

Update Documentation: Ensure the README and other docs are updated if you add new features or change functionality.
